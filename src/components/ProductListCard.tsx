...
const ProductListCard = memo(({ product }: { product: Product }) => {
  ...
  const variantTitle =
    product?.variant && typeof product.variant === 'object' && 'title' in product.variant
      ...
      <div className="flex flex-col sm:flex-row gap-4 p-4">
        ...
        <div className="relative shrink-0 w-full sm:w-48 h-48 bg-gofarm-light-orange/10 rounded-lg overflow-hidden">
            ...
            <Link href={`/product/${product?.slug?.current}`}>
              <Image
                ...
                className={`w-full h-full object-contain transition-transform duration-500 ${
                  product?.stock !== 0 ? 'group-hover:scale-110' : 'opacity-50 grayscale'
                }`}
          ...
          {product?.status && (
            <div className="absolute top-2 left-2 z-10">
              ...
              {product.status === 'new' && (
                <Badge className="bg-gofarm-light-green text-gofarm-white border-0">New</Badge>
              )}
              {product.status === 'sale' && (
                <Badge className="bg-gofarm-green text-gofarm-white border-0">Sale</Badge>
              )}
            </div>
          )}
        ...
        <div className="flex-1 flex flex-col justify-between min-w-0">
          ...
          <div>
            ...
            {/* Description */}
            {product?.description && (
              <p className="text-sm text-gofarm-gray line-clamp-2 mb-3">{product.description}</p>
            )}
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              {product?.totalReviews !== undefined && product.totalReviews > 0 && (
                <span className="text-xs text-gofarm-gray">({product.totalReviews} reviews)</span>
              )}
            </div>
